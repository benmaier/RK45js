...
//
// See the "rk45_sample.html" file for an example of getting started with a one
...
var rk45 = function() {
    ...
    this.debug = false;     // Whether to show the debug messages in the Console.
    this.standard_h = null;
}
...
rk45.prototype = {
    ...
    setH:        function( h ) { this.standard_h = h; return this.h = h; },
    setTol:      function( tol ) { return this.tol = tol; },
    setMaxCount: function( maxCount ) { return this.maxCount = maxCount; },
    resetStatus: function () { return this.status = {success: false, state: 'init', message: null}; },

    reset:       function () {
        this.resetStatus();
        if (this.standard_h !== null)
          this.setH(this.standard_h)
        else
          this.setH(0.1);
    },

    adoptCurrentState: function () {
        this.reset();
        this.setStart(this.stop);
        this.setInitX(this.newX.slice());
    },

    solveTimes: function( times, doCheckSetup=false ) {
      let result = [];
      let saveX = this.x0.slice();
      let saveStart = this.start;
      let saveStop = this.stop;
      let saveH = this.h;

      this.setStart(times[0]);

      result.push(this.x0.slice());
      
      for (let i=1; i<times.length; ++i)
      {

        this.setStop(times[i]);

        this.solve(doCheckSetup);
        result.push(this.newX.slice());

        this.adoptCurrentState();
      }

      this.reset();
      this.setInitX(saveX);
      this.setStart(saveStart);
      this.setH(saveH);
      return result;
    },
    ...
    solve:      function(doCheckSetup=true) {

        // Check for inconsistencies in user inputs.
        if ( (doCheckSetup) && (this.checkSetUp() != 'ok') ) {
            return;
        }
